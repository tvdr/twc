<template>
    <section class="container">
        ...
        <div class="row">
            <div class="col-xs-12 col-sm-4 col-sm-offset-4 well" v-if="weatherData">
                <div v-if="theWeatherIsGood">HURRAY, It's a fine day, go outside and play!</div>
                <div v-if="!theWeatherIsGood">Just stay inside, brew a coffee and read a book!</div>
                <button type="button" class="btn btn-info"  data-toggle="modal" data-target="#detailModal">Show details</button>
            </div>
            ...
            <div class="col-xs-12 alert alert-danger" v-if="isError">No data available, please check settings or your network connection!</div>


        </div>
        <div id="detailModal" class="modal fade" tabindex="-1">
            <div class="modal-dialog"  v-if="weatherData">
                <div class="modal-content">
                <div class="modal-header">
                    <span v-if="usersettings.useCoordsIfAvailable">Current weather in <b>{{weatherData.name}}</b></span>

                        <span v-if="!usersettings.useCoordsIfAvailable">
                            <span v-if="usersettings.city!=''">Current weather in  <b>{{usersettings.city}}</b></span>
                        </span>
                </div>

                    <div class="modal-body">

                        <b>{{weatherData.weather[0].description}}</b><br>
                        <b>{{weatherData.main.temp}} Celsius</b><br>
                        <b>{{weatherData.main.humidity}} % humidity</b><br>
...
<script>
    ...
    methods: {
        fetchData()
        {
            ...
            if (navigator.geolocation && _this.usersettings.useCoordsIfAvailable) {
                navigator.geolocation.getCurrentPosition(function getDataByLocation(position){
                    console.log(position);
                    $.get('http://api.openweathermap.org/data/2.5/weather?lat='+position.coords.latitude+'&lon='+position.coords.longitude+'&units=metric&appid=ececd26df28e15fd7bd62a9919590b9f').then(function (data) {

                        //console.log(data)
                        _this.loading = false
                        _this.weatherData = data;
                        _this.isGoodWeather(_this.usersettings, data);
                    },function(){
                        _this.loading = false;
                        _this.isError = true;
                    })
                ...
                $.get('http://api.openweathermap.org/data/2.5/weather?q='+_this.usersettings.city+'&units=metric&appid=ececd26df28e15fd7bd62a9919590b9f').then(function (data) {
        ...
        isGoodWeather(usersettings, weatherdata)
        {
            ...
            if (weatherdata.main.temp < usersettings.temp.min || weatherdata.main.temp > usersettings.temp.max) {
                tempIsGood = false;
                console.log('tempisnotgood')
            }
            if (weatherdata.main.humidity < usersettings.humidity.min || weatherdata.main.humidity > usersettings.humidity.max) {
                humIsGood = false;
                console.log('humisnotgood')
            }


            if (!tempIsGood || !humIsGood) {
                this.theWeatherIsGood = false;
            }
